/**
 * ============================================================================
 * DRAFT. NOT REVIEWED BY COUNSEL. DO NOT TREAT AS LEGAL ADVICE.
 * ============================================================================
 *
 * The volunteer waiver, signed on a phone at the lot before a shift starts.
 *
 * This text has NOT been reviewed by a lawyer. It was drafted to mirror the
 * vendor agreement's structure and to give counsel something concrete to mark
 * up rather than a blank page. Nothing in it should be relied on as enforceable
 * until a Texas attorney has read it and said so. The page says the same thing
 * on screen, from isDraft below, so a volunteer can see it too.
 *
 * WHAT COUNSEL NEEDS TO LOOK AT, in rough order of how much it matters:
 *
 *   1. Section 9, minors. Texas courts have significantly limited the
 *      enforceability of pre injury releases signed by a parent on a minor's
 *      behalf. Counsel needs to decide what this section can actually
 *      accomplish, whether the operation should accept volunteers under 18 at
 *      all, and whether an indemnity from the parent is the more useful
 *      instrument than a release from the minor. The site currently collects
 *      both. If the answer is that none of it holds, the product decision is to
 *      stop taking minors, and that is a decision for Robert and counsel rather
 *      than something this file should quietly assume either way.
 *   2. Section 5, the release. Drafted for the Texas fair notice doctrine:
 *      express negligence, stated inside the clause itself, and
 *      conspicuousness, which the rendering handles by marking these blocks.
 *      Counsel should confirm both are satisfied as rendered on a phone screen,
 *      not only as written here.
 *   3. Whether a volunteer collecting cash on Coyoteville's behalf creates an
 *      agency relationship that section 3 does not adequately disclaim.
 *   4. Whether the Released Parties definition should match the vendor
 *      agreement word for word or be narrowed for volunteers.
 *
 * HOW TO CHANGE IT. Everything a lawyer would touch is in this one file, so a
 * markup comes back as one edit. But the edit is a NEW FILE, not a change to
 * this one: copy it, give it the next version string, point current.ts at it
 * and add it to registry.ts. Rows already signed keep pointing here and this
 * text has to keep existing for them.
 *
 * NEVER EDIT THIS FILE ONCE ANYBODY HAS SIGNED IT.
 *
